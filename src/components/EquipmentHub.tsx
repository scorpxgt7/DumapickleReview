...
import { INITIAL_PADDLES } from '../data/equipment';
import { 
  Sparkles, Award, ArrowRight, RefreshCw, ShoppingCart, 
  Flame, ShieldCheck, Heart, CircleDot
} from 'lucide-react';
                ...
                <div className="p-6 rounded-3xl bg-slate-900 border border-emerald-500/20 grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                  ...
                  <div className="lg:col-span-8 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                      <a 
                        href={`${recommendedPaddle.affiliateLink}&affsrc=dumapickle`}
                        target="_blank"
                      ...
                      >
                        <ShoppingCart className="w-3.5 h-3.5" /> Buy on Shopee (Affiliate Direct)
                      </a>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h3 className="text-xl font-display font-bold text-slate-900">Equipment Review Database</h3>
            <p className="text-xs text-slate-500">Compare multi-dimensional twist weight and stability stats</p>
          </div>
          <span className="text-xs text-slate-400">Showing {paddles.length} Performance Paddles</span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="equipment-grid-container">
          {paddles.map(paddle => (
            <div key={paddle.id} className="bg-white rounded-3xl border border-slate-100 p-5 flex flex-col justify-between hover:shadow-md hover:border-slate-200 transition-all gap-5">
              
              <div className="flex gap-4">
                {/* Image */}
                <div className="w-24 h-24 rounded-2xl bg-slate-50 overflow-hidden shrink-0 relative">
                  <img 
                    src={paddle.image} 
                    alt={paddle.name}
                    referrerPolicy="no-referrer"
                ...
                <div className="space-y-1">
                  ...
                  <h4 className="font-display font-bold text-slate-900 text-base">{paddle.name}</h4>
                  <p className="text-xs font-semibold text-slate-500">₱{paddle.pricePhp.toLocaleString()}</p>
              ...
              <div className="bg-slate-50 p-3.5 rounded-2xl space-y-2.5">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-200/40 text-[9px] text-slate-500 font-semibold">
              ...
              {/* Shopee affiliate checkout */}
              <div className="flex justify-end pt-1">
                <a 
                  href={`${paddle.affiliateLink}&affsrc=dumapickle`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="py-2 px-4 rounded-xl border border-slate-200 hover:border-slate-900 text-slate-800 font-bold text-xs transition-colors flex items-center gap-2"
                >
                  <ShoppingCart className="w-3.5 h-3.5 text-slate-500" /> Shop Affiliate Links
                </a>
              </div>
